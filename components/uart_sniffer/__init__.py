...
CONFIG_SCHEMA = cv.Schema({cv.GenerateID(): cv.declare_id(Sniffer),cv.Required("rx_a"):cv.use_id(uart.UARTComponent),cv.Required("rx_b"):cv.use_id(uart.UARTComponent),cv.Required("password"):cv.string,cv.Optional("bridge", default=False):cv.boolean,cv.Optional("monitor_only", default=False):cv.boolean}).extend(cv.COMPONENT_SCHEMA)
