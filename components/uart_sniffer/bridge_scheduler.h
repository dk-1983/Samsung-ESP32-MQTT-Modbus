...
// Own traffic is deliberately limited to one power READ per boot.
// Directions identify the receiver: 0 main board, 1 factory Wi-Fi module.
class Scheduler {
 public:
  enum class Result { IDLE, QUEUED, WAITING, CONFIRMED, TIMEOUT, BUSY, CANCELLED, INVALID };
  ...
  const char *status() const {
    ...
    case Result::CANCELLED:return "cancelled";default:return "invalid_response";}
  }
  ...
  template<class Emit> void tick(uint32_t now,bool uart_idle,Emit emit) {
    ...
    bool found=false;
    for(unsigned i=0;i<256;++i){uint8_t candidate=uint8_t(0xE1+i);if(!counter_seen[candidate]||uint32_t(now-counter_at[candidate])>=30000){counter=candidate;found=true;break;}}
    if(!found)return;
    uint8_t p[]={0xD0,0xC0,2,14,0,0,0,0,0,counter,0xFE,0x12,2,2,2,0,0,0xE0};
    for(size_t i=0;i<sizeof(p)-2;++i)p[sizeof(p)-2]^=p[i];
    result=Result::WAITING;sent_at=now;tombstone=true;emit(0,p,sizeof(p),true);
  }
 private:
  ...
  template<class Emit> void frame(unsigned c,const uint8_t *p,size_t n,uint32_t now,Emit emit){
    ...
    if(c==0&&tombstone&&p[9]==counter&&p[10]==0xFE&&p[11]==0x12&&p[12]==3){
      ++suppressed;
      if(result==Result::WAITING){
        // Require exactly the requested power field and its known value.
