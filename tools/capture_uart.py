...
p=argparse.ArgumentParser();p.add_argument('--host',default='10.0.0.15');p.add_argument('--secrets',required=True);p.add_argument('--seconds',type=int,default=60);p.add_argument('--output',required=True);a=p.parse_args()
...
with Path(a.output).open('w',encoding='utf-8') as f:
 while time.monotonic()<end:
  try:
   ...
   if not j.get('passive'):raise RuntimeError('Not a passive sniffer')
   if boot!=j['boot_id']:
